Habebit Core version 0.11.0 is now available from:

  <https://habebit.org/bin/habebit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Habebit Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer HBT/kB fee to be
...
For example, add the following to `habebit.conf`:
...
related to the blockchain in the habebit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Habebit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Habebit Core will 
...
For an example script see [habebit-submittx](https://github.com/laanwj/habebit-submittx).
...
- #5334 `cf87536` libhabebitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new habebit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only HabebitAmountField
- #5753 `0f44672` Add habebit logo to about screen
...
- #5809 `46bfbe7` Add habebit-cli man page
...
- #5849 `41259ca` contrib/init/habebitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Habebit-Qt.app to Habebit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for habebit-tx)
