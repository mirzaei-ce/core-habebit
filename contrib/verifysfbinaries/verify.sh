...
###   This script attempts to download the signature file SHA256SUMS.asc from habebit.org
...
WORKINGDIR="/tmp/habebit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://habebit.org/bin/"
VERSIONPREFIX="habebit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'habebit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Habebit Core binary release signing key installed?"
   fi
